...
namespace JLGA.Architecture.VisualNovel.VNScript.Parser
{
    public class VNActionParser
    {
        ...
        {
            private Data.VNScript _script;
            public VNIndex Index { get; private set; }
            public VNIndex End { get; }
            public bool Ended => Index == End;

            public IndexRange(Data.VNScript script, VNIndex start, VNIndex end)
            {
                _script = script;
                Index = start;
                End = end;
        ...
        public List<Result> Parse(Data.VNScript script, VNErrorAccumulator errors, VNString indexRange)
        {
        ...
        private void _ParseCharactersToIgnore(Data.VNScript script, IndexRange range)
        {
        ...
        private Result? _ParseAction(Data.VNScript script, VNErrorAccumulator errors, IndexRange range)
        {
        ...
        private ResultActionName _ParseActionNameInternal(Data.VNScript script, IndexRange range)
        {
        ...
        private bool _ParseUntilRightArgumentsBracket(Data.VNScript script, VNErrorAccumulator errors, IndexRange range)
        {
        ...
        private List<VNString> _ParseActionArguments(Data.VNScript script, VNErrorAccumulator errors, IndexRange range, List<VNBracketPair> argumentBrackets)
        {
        ...
        private bool _ParseBetweenArguments(Data.VNScript script, VNErrorAccumulator errors, IndexRange range, bool isFirstArgument)
        {
            _ParseCharactersToIgnore(script, range);
            if (range.Ended)
            {
        ...
        private VNString? _ParseArgument(Data.VNScript script, VNErrorAccumulator errors, IndexRange range, VNBracketPair bracketPair)
        {
